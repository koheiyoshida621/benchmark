...
//  for (int iter = 0; iter < numIterations; iter++) {
  for (int iter = 0; iter < numIterations; iter += numIterations) {
    ...
    getTime("Kernel:: histo_intermediates_kernel start");
    ...
    cudaDeviceSynchronize();
    getTime("Kernel:: histo_intermediates_kernel end");
    
    pb_SwitchToSubTimer(&timers, mains, pb_TimerID_KERNEL);
    
    
    getTime("Kernel:: histo_main_kernel start");
    ...
    cudaDeviceSynchronize();
    getTime("Kernel:: histo_main_kernel end");
    
    pb_SwitchToSubTimer(&timers, finals, pb_TimerID_KERNEL);
    
    getTime("Kernel:: histo_final_kernel start");
    ...
    cudaDeviceSynchronize();
    getTime("Kernel:: histo_final_kernel end");
  }
}//ITER
  pb_SwitchToTimer(&timers, pb_TimerID_IO);
